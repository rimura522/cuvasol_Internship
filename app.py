...
app = Flask(__name__)
UPLOAD_FOLDER = r'C:\Users\91636\Desktop\Cuvasol internship\cuvasol_Internship-role-creation-and-filter\resumes'
...
def load_job_descriptions():
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT role_title, role_description FROM roles')
...
# Initialize job descriptions when the app starts
...
def create_job_description():
    ...
    if job_title and job_description:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('INSERT INTO roles (role_title, role_description) VALUES (%s, %s)', (job_title, job_description))
        conn.commit()
        cursor.close()
        conn.close()
...
def update_role():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('UPDATE roles SET role_title = %s, role_description = %s WHERE role_title = %s', (new_title, new_description, old_title))
    conn.commit()
    cursor.close()
    conn.close()
